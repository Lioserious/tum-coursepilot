...
1. Run `python3 scripts/tumonline.py status`. On an error, stop and walk the student through `docs/1-get-your-token.md`.
...
4. Write the candidates to `data/candidates.txt`, one `CODE  # Module name` per line, and run `python3 scripts/tumonline.py fetch data/candidates.txt --semester <id>`.
5. Drop candidates without dates. Read the exam format from the module plan where TUMonline leaves it empty.
6. Put the student's fixed commitments into `data/busy.json`, then test combinations with `python3 scripts/check_plan.py`. Prefer: no clashes, the student's exam preference, fewer days on campus, their interests.
